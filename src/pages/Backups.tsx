...
import { HardDrive, CheckCircle, XCircle, Clock, Download, RefreshCw, Settings2, Trash2, Shield, CheckSquare, Square, Archive } from "lucide-react";
...
import { exportBackups, exportAllData } from "@/lib/export-utils";
...
export default function Backups() {
    ...
    try {
      for (const p of projects) {
        await createBackup.mutateAsync({
          project_id: p.id, status: "success", backup_type: "manual",
  ...
  const handleDownloadSelected = async () => {
    ...
    setExporting(true);
    try {
      await exportBackups(Array.from(selected));
      toast.success(`${selected.size} גיבויים הורדו בהצלחה!`);
      setSelected(new Set());
    } catch (e: any) { toast.error(e.message); }
    setExporting(false);
  };
  ...
  const handleExportAll = async () => {
    ...
    try {
      await exportAllData();
      toast.success("כל הנתונים יוצאו בהצלחה!");
    } catch (e: any) { toast.error(e.message); }
    setExporting(false);
  };
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button variant="outline" className="border-accent text-accent hover:bg-accent/10" onClick={handleExportAll} disabled={exporting}>
            <Archive className="h-4 w-4 ml-2" /> {exporting ? "מייצא..." : "ייצוא הכל (ZIP)"}
          </Button>
                ...
                {projects.map((p: any) => (
                  <div key={p.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary">
                    <div className="flex items-center gap-3">
                      <Switch checked={!!p.auto_backup_enabled} onCheckedChange={(v) => toggleAutoBackup(p.id, v, p.backup_interval || "weekly")} />
                      <span className="font-medium">{p.name}</span>
                      {p.auto_backup_enabled && <Badge variant="outline" className="border-green-300 text-green-700 text-xs">פעיל</Badge>}
                    </div>
                    <div className="flex items-center gap-2">
                      <Select value={p.backup_interval || "weekly"} onValueChange={(v) => toggleAutoBackup(p.id, !!p.auto_backup_enabled, v)}>
                      ...
                      {p.last_backup_at && <span className="text-xs text-muted-foreground">אחרון: {new Date(p.last_backup_at).toLocaleDateString("he-IL")}</span>}
                    </div>
                  </div>
                ))}
              </div>
            )}
            {autoBackupProjects.length > 0 && <p className="text-xs text-muted-foreground mt-3">{autoBackupProjects.length} פרויקטים עם גיבוי אוטומטי מופעל</p>}
          </CardContent>
        </Card>
      )}

      {/* Backup history with multi-select */}
      <Card className="border-2 border-border">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg">היסטוריית גיבויים</CardTitle>
            {selected.size > 0 && (
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="border-accent text-accent">{selected.size} נבחרו</Badge>
                <Button size="sm" variant="outline" className="border-accent text-accent hover:bg-accent/10" onClick={handleDownloadSelected} disabled={exporting}>
                  <Download className="h-4 w-4 ml-1" /> {exporting ? "מוריד..." : "הורד נבחרים (ZIP)"}
                </Button>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-10">
                    <Checkbox checked={backups.length > 0 && selected.size === backups.length} onCheckedChange={selectAll} />
                  </TableHead>
              ...
              <TableBody>
                {backups.map((backup) => {
                  ...
                  const isSelected = selected.has(backup.id);
                  return (
                    <TableRow key={backup.id} className={isSelected ? "bg-accent/5" : ""}>
                      <TableCell>
                        <Checkbox checked={isSelected} onCheckedChange={() => toggleSelect(backup.id)} />
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-xs">{backup.backup_type === 'auto' ? '🔄 אוטומטי' : '✋ ידני'}</Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleDownloadSingle(backup.id)} disabled={exporting}>
